...
{
    /// <summary>Quadratic superellipsoid.</summary>
    /// <remarks>
    /// Surface definition: x^4 + y^4 + z^4 = 1
    /// </remarks>
    [XSight, Properties(nameof(axes), nameof(center), nameof(material))]
    public sealed class Quart : MaterialShape, IShape
    {
        private Vector center, axes;
        ...
        public Quart(
            ...
            [Proposed("1")] double rx, [Proposed("1")] double ry, [Proposed("1")] double rz,
             IMaterial material)
            : this(new(x0, y0, z0), new Vector(rx, ry, rz), material) { }

        /// <summary>Update the bounding box around this shape.</summary>
        private void RecomputeBounds()
        {
            invRadius = new(1 / axes.X, 1 / axes.Y, 1 / axes.Z);
            invNegRadius = negated ? -invRadius : invRadius;
            bounds = Bounds.Create(
                center.X - axes.X, center.Y - axes.Y, center.Z - axes.Z,
                center.X + axes.X, center.Y + axes.Y, center.Z + axes.Z);
        }
        ...
        bool IShape.ShadowTest(Ray ray)
        {
            ...
            double f = 1 / (dx2 * dx2 + dy2 * dy2 + dz2 * dz2);
            double t4 = f + f; t4 += t4;
            Solver.Solve(
                (gx * dx * dx2 + gy * dy * dy2 + gz * dz * dz2) * t4,
                (gx2 * dx2 + gy2 * dy2 + gz2 * dz2) * 6.0 * f,
                (gx * gx2 * dx + gy * gy2 * dy + gz * gz2 * dz) * t4,
                (gx2 * gx2 + gy2 * gy2 + gz2 * gz2 - 1.0) * f,
                ref roots);
            if (roots.Count == 0)
                return false;
                ...
                return Tolerance.Epsilon <= roots.R0 ?
                    roots.R0 < Math.Sqrt(dx2 + dy2 + dz2) :
                    Tolerance.Epsilon <= roots.R1 && roots.R1 <= Math.Sqrt(dx2 + dy2 + dz2);
        }
        ...
        bool IShape.HitTest(Ray ray, double maxt, ref HitInfo info)
        {
            ...
            double dy = ray.Direction.Y * invRadius.Y, dy2 = dy * dy;
            double dz = ray.Direction.Z * invRadius.Z, dz2 = dz * dz;
            double t = 1 / (dx2 * dx2 + dy2 * dy2 + dz2 * dz2);
            double t4 = t + t; t4 += t4;
            Solver.Solve(
                (gx * dx * dx2 + gy * dy * dy2 + gz * dz * dz2) * t4,
                (gx2 * dx2 + gy2 * dy2 + gz2 * dz2) * 6.0 * t,
                (gx * dx * gx2 + gy * dy * gy2 + gz * dz * gz2) * t4,
                (gx2 * gx2 + gy2 * gy2 + gz2 * gz2 - 1.0) * t,
                ref roots);
            if (roots.Count == 0)
                return false;
            else if (roots.Count == 1)
            ...
            else if (roots.R0 >= Tolerance.Epsilon)
                if (roots.R0 <= maxt)
                    t = roots.R0;
                else
                    return false;
            ...
            (gx, gy, gz) = (info.HitPoint - center).Scale(invRadius);
            info.Normal = new Vector(gx * gx * gx, gy * gy * gy, gz * gz * gz).
        ...
        {
            double t = (location.X - center.X) * invRadius.X;
            double x = t * t * t;
            t = (location.Y - center.Y) * invRadius.Y;
            double y = t * t * t;
            t = (location.Z - center.Z) * invRadius.Z;
            return new Vector(x, y, t * t * t).ScaleNormal(invNegRadius);
        }
        ...
        int IShape.GetHits(Ray ray, Hit[] hits)
        {
            ...
            Solver.Solve(
                (gx * dx * dx2 + gy * dy * dy2 + gz * dz * dz2) * t4,
                (gx2 * dx2 + gy2 * dy2 + gz2 * dz2) * 6 * f,
                (gx * dx * gx2 + gy * dy * gy2 + gz * dz * gz2) * t4,
                (gx2 * gx2 + gy2 * gy2 + gz2 * gz2 - 1) * f,
                ref roots);
            switch (roots.Count)
            {
                case 0:
        ...
        {
            IShape q = new Quart(center, axes, material.Clone(force));
            if (negated)
                q.Negate();
            return q;
        }
